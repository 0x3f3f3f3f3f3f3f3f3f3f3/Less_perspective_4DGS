...
from scene.oursfull import GaussianModel, FreeTimeGSModel
...
# ============================================================================
# FreeTimeGS Renderers
# ============================================================================

def train_freetimegs(viewpoint_camera, pc: FreeTimeGSModel, pipe, bg_color: torch.Tensor,
                     scaling_modifier=1.0, override_color=None, basicfunction=None,
                     GRsetting=None, GRzer=None):
    """
    FreeTimeGS training renderer with linear motion and single-peak temporal opacity.

    Key differences from STGS:
    - Linear velocity motion: μx(t) = μx + v·Δt
    - Temporal opacity: σ(t) = exp(-0.5·((t-μt)/s)^2)
    - Direct SH color evaluation (no MLP decoder)
    - Time-independent rotation and scaling
    """
    ...
    # Compute time offset: Δt = t - μt
    current_time = viewpoint_camera.timestamp
    # Ensure current_time is scalar
    if isinstance(current_time, torch.Tensor):
        if current_time.numel() > 1:
            current_time = current_time.mean().item()
        else:
            current_time = current_time.item()

    delta_t = (current_time - pc._t_center)  # (N, 1) via broadcasting

    # FreeTimeGS: position at current time
    means3D = pc.position_at(delta_t)  # μx(t) = μx + v·Δt

    # FreeTimeGS: combined opacity (spatial × temporal)
    opacity_spatial = pc.get_opacity_spatial  # σ_spatial
    opacity_temporal = pc.temporal_opacity(delta_t)  # σ(t)
    opacity = opacity_spatial * opacity_temporal

    # Time-independent rotation and scaling
    rotations = pc.get_rotation()
    scales = pc.get_scaling

    # SH coefficients for color - reshape for rasterizer
    # Rasterizer expects shs in shape (N, num_sh_coeffs, 3) where num_sh_coeffs = (L+1)^2
    # Our _sh is (N, 3*(L+1)^2), need to reshape to (N, (L+1)^2, 3)
    num_sh_coeffs = (pc.max_sh_degree + 1) ** 2
    shs = pc._sh.reshape(-1, 3, num_sh_coeffs).permute(0, 2, 1).contiguous()  # (N, num_sh_coeffs, 3)
    colors_precomp = None
...
def test_freetimegs(viewpoint_camera, pc: FreeTimeGSModel, pipe, bg_color: torch.Tensor,
                    scaling_modifier=1.0, override_color=None, basicfunction=None,
                    GRsetting=None, GRzer=None):
    """
    FreeTimeGS test/inference renderer (with timing).
    """
    ...
    # Compute time offset
    current_time = viewpoint_camera.timestamp
    # Ensure current_time is a scalar (float) or properly shaped tensor
    if isinstance(current_time, torch.Tensor):
        if current_time.numel() > 1:
            current_time = current_time.mean().item()
        else:
            current_time = current_time.item()
    delta_t = current_time - pc._t_center

    # FreeTimeGS computations
    means3D = pc.position_at(delta_t)
    opacity_spatial = pc.get_opacity_spatial
    opacity_temporal = pc.temporal_opacity(delta_t)
    opacity = opacity_spatial * opacity_temporal
    ...
    # SH coefficients for color - reshape for rasterizer
    num_sh_coeffs = (pc.max_sh_degree + 1) ** 2
    shs = pc._sh.reshape(-1, 3, num_sh_coeffs).permute(0, 2, 1).contiguous()
    colors_precomp = None
