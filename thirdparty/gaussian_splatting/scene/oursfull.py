...
class LinearTrajModel(nn.Module):
    """Linear Trajectory Model - lightweight 4D Gaussian representation with linear motion.

    Based on paper section "任意时空处的线性空间轨迹函数".
    Each Gaussian can appear at arbitrary spacetime with linear spatial trajectory.

    Parameters per Gaussian:
      _xyz: (N,3) initial position μ(0) - position at appearance time t₀
      _t_center: (N,1) appearance time t₀
      _t_scale: (N,1) log s (temporal duration parameter)
      _vel: (N,3) linear velocity v = (vₓ, vᵧ, vᵧ)
      _scaling: (N,3) log-diagonal scales (spatial)
      _rotation: (N,4) unit quaternion R (time-independent)
      _opacity: (N,1) spatial base opacity σᵢ (logit space)
      _sh: (N, 3*(L+1)^2) SH coefficients with DC first (L=3 by default)

    Trajectory: μ(t) = μ(0) + v * Δt, where Δt = t - t₀
    Temporal gate: g(t) = exp(-0.5 * ((t - t₀)/s)²)
    Final opacity: α(x,t) = g(t) * σᵢ * exp(-||x - μ(t)||² / (2*blur²))
    """
    ...
    def __init__(self, sh_degree: int = 3):
        ...
        # Core VECE parameters
        self._xyz = torch.empty(0)
        self._t_center = torch.empty(0)
        self._t_scale = torch.empty(0)  # log s
        self._vel = torch.empty(0)
        self._scaling = torch.empty(0)  # log scale diag
        self._rotation = torch.empty(0)  # quat
        self._opacity = torch.empty(0)  # spatial σ
        self._sh = torch.empty(0)  # SH coeffs (all channels, DC + higher)

        # Book keeping for densification / grads
        self.max_radii2D = torch.empty(0)
        self.xyz_gradient_accum = torch.empty(0)
        self.denom = torch.empty(0)
        self.percent_dense = 0
    ...
    @property
    def get_xyz(self):
        """Return base positions (for compatibility)."""
        return self._xyz

    @property
    def get_scaling(self):
        """Return spatial scales."""
        return self.scaling_activation(self._scaling)

    @property
    def get_opacity_spatial(self):
        """Return spatial opacity component."""
        return self.opacity_activation(self._opacity)

    @property
    def get_opacity(self):
        """Return opacity (spatial only, for compatibility with densification)."""
        return self.opacity_activation(self._opacity)

    def temporal_opacity(self, t):
        """Compute temporal gate function: g(t) = exp(-0.5 * ((t - t₀)/s)²).

        Args:
            t: Current time (scalar or tensor)

        Returns:
            Temporal gate values for each Gaussian
        """
        delta_t = t - self._t_center  # Δt = t - t₀
        s = torch.exp(self._t_scale).clamp_min(1e-6)
        return torch.exp(-0.5 * (delta_t / s) ** 2)

    def position_at(self, t):
        """Compute position at time: μ(t) = μ(0) + v * Δt, where Δt = t - t₀.

        According to paper equation:
        - μ(t) is the position at time t
        - μ(0) is the initial position (self._xyz)
        - v is the linear velocity (self._vel)
        - Δt = t - t₀, where t₀ is the appearance time (self._t_center)

        Args:
            t: Current time (scalar or tensor)

        Returns:
            Positions of Gaussians at time t
        """
        delta_t = t - self._t_center  # Δt = t - t₀
        return self._xyz + self._vel * delta_t

    def get_rotation(self, delta_t=None):
        """Get normalized rotation quaternions (time-independent for LinearTraj)."""
        return self.rotation_activation(self._rotation)

    def covariance_at(self):
        """Compute spatial covariance: Σ = R S S^T R^T."""
        L = build_scaling_rotation(self.get_scaling, self.rotation_activation(self._rotation))
        return L @ L.transpose(1, 2)

    def get_features(self, delta_t):
        """Return SH coefficients (for rasterizer compatibility)."""
        return self._sh

    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        """Initialize from SfM sparse point cloud.

        For LinearTraj:
        - Initial positions μ(0) from pcd.points
        - Appearance times t₀ from pcd.times
        - Velocities v initialized to zeros
        - SH from RGB via RGB2SH
        """
        self.spatial_lr_scale = spatial_lr_scale
        xyz = torch.tensor(np.asarray(pcd.points)).float().cuda()
        rgb = torch.tensor(np.asarray(pcd.colors)).float().cuda()

        # Process times to ensure (N, 1) shape
        times_np = np.asarray(pcd.times)
        times = torch.tensor(times_np).float().cuda()

        # Ensure times is (N, 1) - if already 2D, don't unsqueeze
        if times.dim() == 1:
            times = times.unsqueeze(1)  # (N,) -> (N, 1)
        elif times.dim() == 2 and times.shape[1] != 1:
            times = times[:, :1]  # Take first column if multiple
        # If already (N, 1), keep as is

        N = xyz.shape[0]
        print(f"[LinearTraj] Initializing with {N} points")

        # Scales from knn distance
        dist2 = torch.clamp_min(distCUDA2(xyz), 1e-7)
        scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3).clamp(-10, 1.0)

        # Rotations (identity quaternion)
        rots = torch.zeros((N, 4), device='cuda')
        rots[:, 0] = 1.0

        # Opacity (low initial value)
        opac = inverse_sigmoid(0.1 * torch.ones((N, 1), device='cuda'))

        # Velocity (zero init)
        vel = torch.zeros((N, 3), device='cuda')

        # Temporal scale (log s, init to 0 → s=1)
        t_scale = torch.zeros((N, 1), device='cuda')

        # SH coefficients: DC from RGB, rest zeros
        sh_dc = RGB2SH(rgb)  # (N, 3) - DC component
        num_sh_coeffs = 3 * (self.max_sh_degree + 1) ** 2
        sh_all = torch.zeros((N, num_sh_coeffs), device='cuda')
        sh_all[:, :3] = sh_dc  # First 3 are DC for RGB

        # Register as nn.Parameter
        self._xyz = nn.Parameter(xyz.requires_grad_(True))
        self._t_center = nn.Parameter(times.requires_grad_(True))
        self._t_scale = nn.Parameter(t_scale.requires_grad_(True))
        self._vel = nn.Parameter(vel.requires_grad_(True))
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = nn.Parameter(opac.requires_grad_(True))
        self._sh = nn.Parameter(sh_all.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        """Setup optimizer for LinearTraj parameters."""
        self.percent_dense = training_args.percent_dense

        # Use existing training_args learning rates where applicable
        self.optimizer = torch.optim.Adam([
            {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, 'name': 'xyz'},
            {'params': [self._vel], 'lr': training_args.position_lr_init * self.spatial_lr_scale, 'name': 'vel'},
            {'params': [self._t_center], 'lr': training_args.trbfc_lr, 'name': 't_center'},
            {'params': [self._t_scale], 'lr': training_args.trbfs_lr, 'name': 't_scale'},
            {'params': [self._opacity], 'lr': training_args.opacity_lr, 'name': 'opacity'},
            {'params': [self._scaling], 'lr': training_args.scaling_lr, 'name': 'scaling'},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, 'name': 'rotation'},
            {'params': [self._sh], 'lr': training_args.feature_lr, 'name': 'sh'}
        ], lr=0.0, eps=1e-15)
    ...
    def update_learning_rate(self, iteration):
        """Update learning rate schedule for xyz and vel."""
        for g in self.optimizer.param_groups:
            if g['name'] in ['xyz', 'vel']:
                g['lr'] = self.xyz_scheduler_args(iteration)
    ...
    @torch.no_grad()
    def periodic_relocate(self, iter_idx: int, every: int = 500, grad_w: float = 0.5,
                         opa_w: float = 0.5, opa_th: float = 0.2):
        """Periodic relocation strategy for LinearTraj model.

        Move low-opacity Gaussians to high-demand regions based on:
        s = λg * ∇g + λo * σ

        Args:
            iter_idx: Current iteration
            every: Relocation frequency (default 500)
            grad_w: Weight for gradient term (λg)
            opa_w: Weight for opacity term (λo)
            opa_th: Opacity threshold for "low opacity" selection
        """
        if every <= 0 or iter_idx % every != 0:
            return

        # Compute relocation score
        grads = self.xyz_gradient_accum / self.denom.clamp_min(1)
        grads[torch.isnan(grads)] = 0.0
        score = grad_w * grads.squeeze(-1) + opa_w * self.get_opacity_spatial.squeeze(-1)

        # Find low opacity points
        low_opa_mask = (self.get_opacity_spatial.squeeze(-1) < opa_th)
        num_relocate = low_opa_mask.sum().item()

        if num_relocate == 0:
            return

        # Select top-k high-score positions as relocation targets
        topk = torch.topk(score, k=min(score.numel(), num_relocate), largest=True).indices
        src_xyz = self._xyz[topk]
        src_t = self._t_center[topk]

        # Relocate low-opacity points (with small noise for diversity)
        noise = 1e-3 * torch.randn_like(src_xyz[:num_relocate])
        self._xyz.data[low_opa_mask] = src_xyz[:num_relocate] + noise
        self._t_center.data[low_opa_mask] = src_t[:num_relocate]

        print(f"[LinearTraj] Relocated {num_relocate} low-opacity Gaussians at iteration {iter_idx}")

    # === Densification methods (for adaptive Gaussian control) ===
    def cat_tensors_to_optimizer(self, tensors_dict):
        """Concatenate new tensors to optimizer parameters."""
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if len(group["params"]) == 1 and group["name"] in tensors_dict:
                extension_tensor = tensors_dict[group["name"]]
    ...
    def densification_postfix(self, new_xyz, new_t_center, new_t_scale, new_vel, new_scaling,
                             new_rotation, new_opacity, new_sh):
        """Add new Gaussians to the model."""
        d = {
            "xyz": new_xyz,
            "vel": new_vel,
            "t_center": new_t_center,
            "t_scale": new_t_scale,
            "opacity": new_opacity,
            "scaling": new_scaling,
            "rotation": new_rotation,
        ...
        self._t_center = optimizable_tensors["t_center"]
        self._t_scale = optimizable_tensors["t_scale"]
        self._opacity = optimizable_tensors["opacity"]
        self._scaling = optimizable_tensors["scaling"]
        self._rotation = optimizable_tensors["rotation"]
        self._sh = optimizable_tensors["sh"]

        # Reset bookkeeping for all Gaussians
        self.xyz_gradient_accum = torch.zeros((self._xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self._xyz.shape[0], 1), device="cuda")
        self.max_radii2D = torch.zeros((self._xyz.shape[0]), device="cuda")

    def _prune_optimizer(self, mask):
        """Prune optimizer state to match pruned parameters."""
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
    ...
    def prune_points(self, mask):
        """Remove Gaussians based on mask (True = remove, False = keep)."""
        valid_points_mask = ~mask  # Invert: True = keep

        # Prune parameters using optimizer
        optimizable_tensors = self._prune_optimizer(valid_points_mask)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # Select points: high gradient AND small size
        selected_pts_mask = torch.where(torch.norm(grads, dim=-1) >= grad_threshold, True, False)
        ...
        new_xyz = self._xyz[selected_pts_mask]
        new_t_center = self._t_center[selected_pts_mask]
        new_t_scale = self._t_scale[selected_pts_mask]
        new_vel = self._vel[selected_pts_mask]
        new_scaling = self._scaling[selected_pts_mask]
        new_rotation = self._rotation[selected_pts_mask]
        new_opacity = self._opacity[selected_pts_mask]
        new_sh = self._sh[selected_pts_mask]

        self.densification_postfix(new_xyz, new_t_center, new_t_scale, new_vel,
                                  new_scaling, new_rotation, new_opacity, new_sh)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # Select points: high gradient AND large size
        selected_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
        selected_pts_mask = torch.logical_and(
            selected_pts_mask,
            torch.max(self.get_scaling, dim=1).values > self.percent_dense * scene_extent
        ...
        # Sample N new positions around each selected Gaussian
        stds = self.get_scaling[selected_pts_mask].repeat(N, 1)
        means = torch.zeros((stds.size(0), 3), device="cuda")
        samples = torch.normal(mean=means, std=stds)
        rots = build_rotation(self._rotation[selected_pts_mask]).repeat(N, 1, 1)

        # New positions: rotate samples and add to original position
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self._xyz[selected_pts_mask].repeat(N, 1)

        # Scale down the split Gaussians
        new_scaling = self.scaling_inverse_activation(self.get_scaling[selected_pts_mask].repeat(N, 1) / (0.8 * N))
        new_rotation = self._rotation[selected_pts_mask].repeat(N, 1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N, 1)
        new_sh = self._sh[selected_pts_mask].repeat(N, 1)

        # LinearTraj-specific: inherit temporal parameters and velocity
        new_t_center = self._t_center[selected_pts_mask].repeat(N, 1)
        ...
        # Prune the original large Gaussians
    ...
    def densify_pruneclone(self, max_grad, min_opacity, extent, max_screen_size):
        """Main densification entry: clone + split + prune."""
        grads = self.xyz_gradient_accum / self.denom.clamp_min(1)
        grads[grads.isnan()] = 0.0

        print(f"Before clone: {self._xyz.shape[0]}")
        self.densify_and_clone(grads, max_grad, extent)
        print(f"After clone: {self._xyz.shape[0]}")

        self.densify_and_split(grads, max_grad, extent, N=2)
        print(f"After split: {self._xyz.shape[0]}")

        # Prune low-opacity and oversized Gaussians
        prune_mask = (self.get_opacity_spatial < min_opacity).squeeze()
        if max_screen_size:
            big_points_vs = self.max_radii2D > max_screen_size
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
            prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
        ...
        print(f"After prune: {self._xyz.shape[0]}")

        torch.cuda.empty_cache()

    def reset_opacity(self):
        """Reset opacity to help prevent overfitting."""
        opacities_new = self.inverse_opacity_activation(
            torch.min(self.get_opacity_spatial, torch.ones_like(self.get_opacity_spatial) * 0.01))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]

    def construct_list_of_attributes(self):
        """Construct attribute names for PLY export."""
        names = ['x', 'y', 'z', 't_center', 't_scale', 'nx', 'ny', 'nz']
        names += ['vel_x', 'vel_y', 'vel_z']
        names += [f'scale_{i}' for i in range(self._scaling.shape[1])]
        names += [f'rot_{i}' for i in range(self._rotation.shape[1])]
        names += ['opacity']
        names += [f'sh_{i}' for i in range(self._sh.shape[1])]
        return names

    def save_ply(self, path):
        """Save LinearTraj model to PLY format."""
        mkdir_p(os.path.dirname(path))

        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        t_center = self._t_center.detach().cpu().numpy()
        ...
        sh = self._sh.detach().cpu().numpy()

        dtype_full = [(n, 'f4') for n in self.construct_list_of_attributes()]
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate(
            (xyz, t_center, t_scale, normals, vel, scale, rot, opac, sh), axis=1)
        elements[:] = list(map(tuple, attributes))

        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)

        print(f"[LinearTraj] Saved {xyz.shape[0]} Gaussians to {path}")

    def load_ply(self, path):
        """Load LinearTraj model from PLY format."""
        plydata = PlyData.read(path)
        ...
        t_center = np.asarray(plydata.elements[0]["t_center"])[..., np.newaxis]
        t_scale = np.asarray(plydata.elements[0]["t_scale"])[..., np.newaxis]
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
        ...
        sh = np.zeros((xyz.shape[0], len(sh_names)))
        for idx, attr_name in enumerate(sh_names):
            sh[:, idx] = np.asarray(plydata.elements[0][attr_name])

        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True))
        self._t_center = nn.Parameter(torch.tensor(t_center, dtype=torch.float, device="cuda").requires_grad_(True))
        ...
        self.active_sh_degree = self.max_sh_degree
        print(f"[LinearTraj] Loaded {xyz.shape[0]} Gaussians from {path}")

    def replace_tensor_to_optimizer(self, tensor, name):
        """Replace a parameter in the optimizer (for densification operations)."""
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if group["name"] == name:
                stored_state = self.optimizer.state.get(group['params'][0], None)
